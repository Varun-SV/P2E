...
with tab1:
    ...
    with col1:
        ...
        if main_script:
            st.session_state.main_file = {
                ...
                'content': main_script.read().decode('utf-8')
            }
        ...
        if additional_files:
            ...
            for file in additional_files:
                ...
                st.session_state.additional_files.append({
                    'name': file.name,
                    'content': content,
                    'is_binary': file.type.startswith('image') or file.name.endswith('.ico')
                })
            st.success(f"✅ Loaded {len(additional_files)} file(s)")
    ...
    with col_req2:
        ...
        if req_file:
            requirements_input = req_file.read().decode('utf-8')
...
'''
                ...
                with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                    ...
                    if icon_file:
                        zip_file.writestr('icon.ico', icon_file.read())
